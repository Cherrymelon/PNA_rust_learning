...
fn new_log_file(path: &Path, gen: u64,
    ...
    let writer = BufWriterWithPos::new(OpenOptions::new().create(true)
                                                         .write(true)
                                                         .append(true)
                                                         .open(&path)?)?;
...
fn sorted_gen_list(path: &Path) -> Result<Vec<u64>> {
    let mut gen_list: Vec<u64> =
        fs::read_dir(&path)?.flat_map(|res| -> Result<_> { Ok(res?.path()) })
                            .filter(|path| {
                                path.is_file()
...
fn load(gen: u64, reader: &mut BufReaderWithPos<File>,
